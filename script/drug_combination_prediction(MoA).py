...
class Drug_Combination:
    ...
    def load_prism_moa(self):
        self.prism_unique_moa_name = pd.read_csv(args.depmap_prism)
    ...
    def merge_moa_prism(self):
        ...
        moa_merge_df = moa_merge_df[~moa_merge_df['moa'].isnull()]
    ...
    def compute_synergy_score_moa_prism(self):
        ...
        chunks_per_range = 10
        chunk_ranges = [(i, min(i + chunks_per_range, num_chunks))
                        for i in range(0, num_chunks, chunks_per_range)]

        print(f"Generated chunk ranges: {chunk_ranges}")
